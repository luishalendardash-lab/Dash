...
begin
  ...
  -- link de destino do quiz fica junto do quiz, não numa config solta
  if p ? 'grupo_url' then
...
returns jsonb language plpgsql security definer
set search_path = dash, public as $$
declare v_lanc uuid; v_res jsonb; v_resp int; v_grupo text;
begin
  select id, config->>'grupo_url' into v_lanc, v_grupo
  from dash.lancamentos where slug = p->>'lancamento';
  ...
  return jsonb_build_object('ok', true, 'perguntas', coalesce(v_res, '[]'::jsonb),
                            'ja_responderam', v_resp, 'grupo_url', v_grupo);
end $$;
